...
public class PrototypeTest {
    public static void main(String[] args) {
        ...
        System.out.println(copyFactory.cachePrototypes);
    }
}

class CopyFactory {
    static Map<String, Car> cachePrototypes = new HashMap<>();

    public Car getNewCopy(Car car) {
        if (cachePrototypes.containsValue(car)) {
            Car cacheCar = cachePrototypes.get(car.getName());
            return cacheCar.copy();
        } else {
            cachePrototypes.put(car.getName(), car);
            return car.copy();
        }
    }
}
